...
// Loads the persisted ring buffer state from NVS into RAM once, then keeps
// NVS closed for the rest of normal operation.
// ---------------------------------------------------------------------------
...
void TripLogger::update(int speed_rpm, int udc_dv, int idc_da,
    ...
    // Safely auto-sync to flash periodically to protect against accidental power cut
    if (now - _lastSyncTime >= TRIPLOG_SYNC_INTERVAL_MS) {
        sync();
    }
}
...
// ---------------------------------------------------------------------------
// Helpers — legacy slot accessors, now operate on the RAM cache directly.
// Kept for API compatibility in case anything still calls them.
// ---------------------------------------------------------------------------
